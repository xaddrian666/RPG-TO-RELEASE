  ...
  this.loadPlayerData = function() {
    return new Promise(resolve => {
      ...
      game.socket.on('retrieve userdata', function(res) {
        console.log(`nowy user: ${res}`);
        let rn = Math.floor(Math.random() * 1000);
        game.player.characters = [];
        $.each(res, function(i, v) {
          game.player.characters.push({name: v.character_name+rn, id: v.slot_id, sprite: v.sprite_id, x:v.pos_x, y:v.pos_y, location:v.location});
        });
        // char_id = prompt();
        char_id = 1;
        ...
        game.player.character.name = game.player.characters[char_id].name+rn;
      ...
      game.socket.on('get players', function(arr) {
        $.each(arr, function(i, v) {
          if(v.character.name != game.player.character.name) {
            //v.draw = game.player.draw;
            //v.character.sprite.load();
            game.players.push(v);
          }
        });
      });

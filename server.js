...
function onDisconnect(socket, player) {
  console.log(`${player.character.name} disconnected`);
  id = getElementByIdInArray(online_players, player, "character.name");
  el = online_players[id];
  con.query(`UPDATE characters SET pos_x = '${el.position.x}', pos_y = '${el.position.y}', location = '${el.location.name}' WHERE character_name='${el.character.name}'`, function (err, result) {
  if (err) throw err;
...
function retrieveLocation(io, val) {
  val = val.toLowerCase();
  io.emit('retrieve location', locations[val]);
}

io.on('connection', function(socket){
  //SEND LOCATION INFO TO PLAYER
  socket.on('get location data', function(val) {
    retrieveLocation(io, val);
  });
  socket.on('player joined', function(player) {
    ...
    console.log(`${player.character.name} connected`);
    io.emit('chat message', player.character.name+" has joined the game");
  ...
  socket.on('update player info', function(type, data) {
    id = getElementByIdInArray(online_players, data.name, "character.name");
    if(type == "position") {
  ...
  socket.on('get userdata from db', function(name) {
    con.query(`SELECT users.name, characters.character_name, characters.slot_id, characters.sprite_id, characters.pos_x, characters.pos_y, characters.location FROM users JOIN characters ON users.name = characters.owner_name WHERE name="${name}"`, function (err, result) {
    if (err) throw err;
      socket.emit('retrieve userdata', result);
    });
  });
